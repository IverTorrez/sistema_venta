...
define("DB_USER", 'root');
define("DB_PASSWORD", 'celiaquispe');
define("DB_NAME", 'sistema_venta');
...
class Backup_Database {
    ...
    public function __construct($host, $username, $passwd, $dbName, $charset = 'utf8') {
        ...
        $this->backupFile              = 'myphp-backup-'.$this->dbName.'-'.date("Y-m-d_H-i-s", time()).'.sql';

        $_SESSION['archivo'] = 'myphp-backup-'.$this->dbName.'-'.date("Y-m-d_H-i-s", time()).'.sql';
    ...
    public function backupTables($tables = '*') {
        try {
          ...
          //COMENTAMOS PARA QUE AL CREAR EL BACKUP NO CREE AUTOMATICAMENTE UNA BASE DE DATOS 
    ...
    public function obfPrint ($msg = '', $lineBreaksBefore = 0, $lineBreaksAfter = 1) {
       ...
       // echo $output; ESTAC LINEA DE CODIGO MUESTRA TODAS LAS TABLAS QUE SE ESTAN HACIENDO BACKUP
...
$url='myphp-backup-files/'.$_SESSION['archivo'].'.gz';
echo "<script >
  {
    location.href='$url'
